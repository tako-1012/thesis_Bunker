#!/usr/bin/env python3
"""
Run FieldD*Hybrid on dataset3 - SINGLE THREAD VERSION
"""
import sys
...
def run_fieldd_on_scenario(scenario, timeout=120):
    """Run FieldD*Hybrid on a single scenario"""
...
def main():
    ...
    print(f"Running FieldD*Hybrid on {len(scenarios)} scenarios (SINGLE THREAD)")
    print(f"Timeout: 120 seconds per task\n")

    results = []
    output_dir = Path('benchmark_results')
    output_dir.mkdir(exist_ok=True)
    output_file = output_dir / 'dataset3_fieldd_final_results.json'
    ...
    for i, scenario in enumerate(scenarios):
        ...
        result = run_fieldd_on_scenario(scenario, timeout=120)
    ...
    print(f"\n{'='*60}")
    print(f"FieldD*Hybrid Benchmark Complete!")
