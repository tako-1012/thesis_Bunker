#!/usr/bin/env python3
"""
Run TA* (TerrainAwareAStar) on dataset3 - SINGLE THREAD VERSION
"""
import sys
...
def run_tastar_on_scenario(scenario, timeout=120):
    ...
    try:
        ...
        return {
            ...
            'scenario_id': sid,
            'environment_type': scenario.get('env'),
            'map_size': int(scenario.get('map_size', 64)),
            'success': False,
            'computation_time': float(elapsed),
...
def main():
    ...
    print(f"Running TA* on {len(scenarios)} scenarios (SINGLE THREAD)")
    ...
    output_file = output_dir / 'dataset3_tastar_final_results.json'
    ...
    for i, scenario in enumerate(scenarios):
        ...
        result = run_tastar_on_scenario(scenario, timeout=120)
