#!/usr/bin/env python3
"""
Run RRT* on dataset3 - SINGLE THREAD VERSION
"""
import sys
...
from path_planner_3d.rrt_star_planner_3d import RRTStarPlanner3D
...
def run_rrt_on_scenario(scenario, timeout=120):
    """Run RRT* on a single scenario"""
    ...
    try:
        ...
        planner = RRTStarPlanner3D(voxel_size=1.0)
        result = planner.plan_path(start, goal, terrain_data=terrain_data, voxel_grid=voxel_grid, timeout=timeout)
...
def main():
    ...
    print(f"Running RRT* on {len(scenarios)} scenarios (SINGLE THREAD)")
    print(f"Timeout: 120 seconds per task\n")

    results = []
    output_dir = Path('benchmark_results')
    output_dir.mkdir(exist_ok=True)
    output_file = output_dir / 'dataset3_rrt_final_results.json'
    ...
    for i, scenario in enumerate(scenarios):
        ...
        result = run_rrt_on_scenario(scenario, timeout=120)
    ...
    print(f"\n{'='*60}")
    print(f"RRT* Benchmark Complete!")
