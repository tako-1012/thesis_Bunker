...
class DijkstraPlanner3D:
    ...
    def __init__(self, grid_size=(100, 100, 30), voxel_size=0.1, max_slope=30.0,
        ...
        logger.info(f"Dijkstra initialized: map_size={self.map_size:.1f}m, "
    ...
    def plan_path(self, start, goal, terrain_data=None, timeout=None):
        ...
        Args:
            start: スタート位置 [x, y, z]
            goal: ゴール位置 [x, y, z]
            terrain_data: 地形データ（Dijkstraでは未使用、API統一のため）
        ...
        logger.info(f"Dijkstra planning: {start} -> {goal}, timeout={timeout}s")
        ...
        try:
            ...
            max_search_distance = goal_distance * 3.0  # 探索範囲制限
            
            while open_set:
                # タイムアウトチェック
                if time.time() - start_time > timeout:
                    logger.warning("Dijkstra timeout")
                ...
                if self._is_goal(current, goal):
                    ...
                    logger.info(f"Dijkstra success! nodes={nodes_explored}, "
                ...
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        for dz in [-1, 0, 1]:
                            ...
                            heapq.heappush(open_set, (tentative_g, id(neighbor_node), neighbor_node))
