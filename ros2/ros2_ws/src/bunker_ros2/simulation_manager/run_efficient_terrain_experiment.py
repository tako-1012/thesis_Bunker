"""
Phase 2改訂版: 効率的な代表的地形実験

戦略変更：
- Dijkstraを除外（時間がかかりすぎる）
- 4アルゴリズム（A*, Weighted A*, RRT*, TA-A*）で比較
- 各地形10シナリオに削減
- 総実験数: 4地形 × 10シナリオ × 4アルゴリズム = 160実験
"""
import json
import time
import numpy as np
...
class EfficientTerrainExperiment:
    """効率的な代表的地形実験"""
    
    def __init__(self):
        # 4つの地形タイプ（Extreme Hazardsは改良版で使用）
        self.terrain_types = [
        ...
        self.terrain_names = {
            ...
            'extreme_hazards': 'Extreme Hazards (Improved)'
        }
    ...
    def run_all_experiments(self):
        ...
        logger.info("="*70)
        logger.info("Phase 2改訂版: 効率的な代表的地形実験")
        logger.info("="*70)
        logger.info(f"地形数: {len(self.terrain_types)}")
        logger.info(f"アルゴリズム数: 4 (Dijkstra除外)")
        logger.info("="*70)
    ...
    def _run_terrain_experiments(self, terrain_type: str) -> Dict:
        ...
        # 最大10シナリオに制限
        scenario_files = scenario_files[:10]
        logger.info(f"使用シナリオ数: {len(scenario_files)}")
        
        # アルゴリズム初期化（Dijkstra除外）
    ...
    def _save_results(self):
        """結果を保存"""
        output_path = Path('../results/efficient_terrain_results.json')
        ...
        output_data = {
            ...
            'metadata': {
                ...
                'note': 'Dijkstra excluded due to excessive computation time'
            }
        }
    ...
    def _print_final_summary(self):
        ...
        logger.info(f"\n{'='*70}")
        logger.info("Phase 2改訂版 完了！最終サマリー")
        logger.info(f"{'='*70}")
        ...
        # テーブル形式で表示
        print(f"\n地形タイプ別成功率:")
        print(f"{'Terrain':25s} | {'A*':10s} | {'W-A*':10s} | {'RRT*':10s} | {'TA-A*':10s}")
        print("-"*75)
        ...
        logger.info(f"\n✅ Phase 2改訂版完了！")
        logger.info(f"結果: ../results/efficient_terrain_results.json")

if __name__ == '__main__':
    experiment = EfficientTerrainExperiment()
