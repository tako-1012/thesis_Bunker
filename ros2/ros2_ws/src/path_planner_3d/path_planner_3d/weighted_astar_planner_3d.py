...
class WeightedAStarPlanner3D:
    """Weighted A*法による3D経路計画"""
    
    def __init__(self, grid_size=(100, 100, 30), voxel_size=0.1, max_slope=30.0,
                 epsilon=1.5, map_bounds=None):
        """
        初期化
        ...
        logger.info(f"Weighted A* initialized: map_size={self.map_size:.1f}m, "
                   f"goal_threshold={self.goal_threshold:.2f}m, epsilon={epsilon}")
    ...
    def plan_path(self, start, goal, terrain_data=None, timeout=None):
        ...
        Args:
            start: スタート位置 [x, y, z]
            goal: ゴール位置 [x, y, z]
            terrain_data: 地形データ（Weighted A*では未使用、API統一のため）
        ...
        logger.info(f"Weighted A* planning: {start} -> {goal}, timeout={timeout}s")
        ...
        try:
            ...
            max_search_distance = goal_distance * 2.5  # 探索範囲制限
            
            while open_set:
                # タイムアウトチェック
                if time.time() - start_time > timeout:
                    logger.warning("Weighted A* timeout")
                ...
                if self._is_goal(current, goal):
                    ...
                    logger.info(f"Weighted A* success! nodes={nodes_explored}, "
                ...
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        for dz in [-1, 0, 1]:
                            ...
                            f_score = tentative_g + self.epsilon * h_score
            ...
            # 経路が見つからなかった
            logger.warning("Weighted A*: No path found")
            return PlanningResult(
                success=False,
                path=[],
                computation_time=time.time() - start_time,
            ...
            logger.error(f"Weighted A* error: {e}")
            import traceback
            traceback.print_exc()
