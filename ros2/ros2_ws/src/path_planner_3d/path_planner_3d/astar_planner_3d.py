...
class AStarPlanner3D:
    ...
    def __init__(
        ...
        Args:
            ...
            map_bounds: マップ境界（辞書）. None の場合はデフォルトを設定
        """
    ...
    def _get_surface_height(self, x: int, y: int) -> Optional[int]:
        """指定(x,y)位置の地表面の高さ（z座標）を取得"""
        if self.voxel_grid is None:
            return None
        
        # 上から順に占有ボクセルを探す
        for z in range(self.grid_size[2] - 1, -1, -1):
            if self.voxel_grid[x, y, z] > 0.5:  # 占有閾値
                return z
    ...
    def _calculate_risk_at(self, pos: Tuple[int, int, int]) -> float:
        """指定位置のリスクスコアを計算（0.0～1.0）"""
        if self.voxel_grid is None:
            return 0.0
        
        x, y, z = pos
        
        # 周囲の障害物密度をチェック（3x3x3の範囲）
        obstacle_count = 0
        total_voxels = 0
        ...
        risk = obstacle_count / total_voxels
        return risk
