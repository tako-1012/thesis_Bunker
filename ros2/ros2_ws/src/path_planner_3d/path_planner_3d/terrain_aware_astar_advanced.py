...
class TerrainAwareAStar:
    ...
    def __init__(
        ...
        logger.info(f"TA* initialized: analysis_radius={terrain_analysis_radius}, "
               f"learning={enable_online_learning}, heuristic_weight={heuristic_weight}")
    ...
    def set_terrain_data(self, voxel_grid, terrain_data) -> None:
        ...
        # 地形複雑度マップを事前計算
        logger.info("Computing terrain complexity map...")
        self._precompute_terrain_maps()
        logger.info("Terrain complexity map computed")
    ...
    def plan_path(
        ...
        # 境界チェック緩和: グリッド外なら最近傍にクランプ
        if not self._is_in_grid(start_idx):
            logger.warning(f"Start {start_idx} is outside grid, clamping.")
            start_idx = self._clamp_to_grid(start_idx)
        if not self._is_in_grid(goal_idx):
            logger.warning(f"Goal {goal_idx} is outside grid, clamping.")
            goal_idx = self._clamp_to_grid(goal_idx)

        logger.info(f"TA* planning: start={start_idx}, goal={goal_idx}")

        # 傾斜制約リスト（初期値→緩和）
        slope_limits = [30.0, 35.0, 40.0, 50.0, 1e6]  # 最後は傾斜無制限
        original_is_traversable = self._is_traversable

        for max_slope in slope_limits:
            def is_traversable_with_limit(pos, max_slope=max_slope):
            ...
            if path_indices is not None:
                ...
                # 元のis_traversableに戻す
                self._is_traversable = original_is_traversable
                path_world = [self._voxel_to_world(idx) for idx in path_indices]
                self.last_search_stats['path_length'] = len(path_world)
                return path_world
            else:
                logger.info(f"TA* failed with max_slope={max_slope}, trying next relaxation...")
        ...
        if path_indices is not None:
            ...
            return path_world
        self._is_traversable = original_is_traversable
        logger.warning("TA* failed for all slope/iteration relaxations")
        return None
    ...
    def _terrain_aware_search(
        ...
        while open_list and nodes_explored < max_iterations:
            ...
            if self.terrain_complexity_map is not None:
                ...
                cx, cy = self._clamp_to_grid((x, y, 0))[:2]
                complexity_sum += self.terrain_complexity_map[cx, cy]
            ...
            for neighbor_pos in self._get_neighbors(current.position):
                ...
                tentative_f = tentative_g + self.heuristic_weight * neighbor.h_cost

                # より良い経路が見つかった場合
                if tentative_g < neighbor.g_cost or not hasattr(neighbor, 'f_cost'):
                    neighbor.parent = current
                    neighbor.g_cost = tentative_g
                    neighbor.f_cost = tentative_f
    ...
    def _calculate_slope_penalty(self, pos: Tuple[int, int, int]) -> float:
        """傾斜ペナルティを計算（非線形・非対称型、最大傾斜超過時は大ペナルティ）"""
        slope_deg = self._get_slope_at(pos)
        max_slope = 40.0  # plan_pathの緩和上限と合わせる
        if slope_deg < 10.0:
            return slope_deg * 0.01
        elif slope_deg < 20.0:
            # 緩やかな増加
            return 0.1 + (slope_deg - 10.0) * 0.07
        elif slope_deg < 30.0:
            # 急増
            return 0.8 + (slope_deg - 20.0) * 0.2
        elif slope_deg < max_slope:
            # 30度超えはさらに急増
            return 2.8 + (slope_deg - 30.0) * 1.0
        else:
            # 最大傾斜超過は極大コスト
            return 1000.0
    ...
    def _calculate_roughness_penalty(self, pos: Tuple[int, int, int]) -> float:
        ...
        x, y = pos[0], pos[1]
        cx, cy = self._clamp_to_grid((x, y, 0))[:2]
        complexity = self.terrain_complexity_map[cx, cy]
    ...
    def _get_terrain_type(self, pos: Tuple[int, int, int]) -> TerrainType:
        ...
        x, y = pos[0], pos[1]
        cx, cy = self._clamp_to_grid((x, y, 0))[:2]
        return self.terrain_type_map[cx, cy]
    ...
    def _is_in_grid(self, pos: Tuple[int, int, int]) -> bool:
        """グリッド範囲内チェック（端の1セル外も許容）"""
        x, y, z = pos
        margin = 1
        return (-margin <= x < self.grid_size[0] + margin and
                -margin <= y < self.grid_size[1] + margin and
                -margin <= z < self.grid_size[2] + margin)

    def _clamp_to_grid(self, pos: Tuple[int, int, int]) -> Tuple[int, int, int]:
        x, y, z = pos
        x = min(max(x, 0), self.grid_size[0] - 1)
        y = min(max(y, 0), self.grid_size[1] - 1)
        z = min(max(z, 0), self.grid_size[2] - 1)
        return (x, y, z)
    ...
    def _get_surface_height(self, x: int, y: int) -> Optional[int]:
        ...
        for z in range(self.grid_size[2] - 1, -1, -1):
            cx, cy, cz = self._clamp_to_grid((x, y, z))
            if self.voxel_grid[cx, cy, cz] > 0.5:
                return z
    ...
    def _is_obstacle_at(self, pos: Tuple[int, int, int]) -> bool:
        ...
        if self.voxel_grid is None:
            ...
            x, y, z = pos
            cx, cy, cz = self._clamp_to_grid((x, y, z))
            result = self.voxel_grid[cx, cy, cz] > 0.5
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                for dz in [-1, 0, 1]:
                    ...
                    cnx, cny, cnz = self._clamp_to_grid((nx, ny, nz))
                    total_voxels += 1
                    if self.voxel_grid[cnx, cny, cnz] > 0.5:
                        obstacle_count += 1
