...
class PlannerBenchmark:
    ...
    def _generate_simple_terrain(self, seed: int, grid_size: int = 100) -> np.ndarray:
        ...
        """
        np.random.seed(seed)
        cost_map = np.ones((grid_size, grid_size), dtype=np.float32) * 0.05  # ベースコストさらに低減
        noise = np.random.normal(0, 0.01, (grid_size, grid_size))  # ノイズ幅も縮小
    ...
    def _generate_moderate_terrain(self, seed: int, grid_size: int = 100) -> np.ndarray:
        ...
        """
        np.random.seed(seed)
        cost_map = np.ones((grid_size, grid_size), dtype=np.float32) * 0.12  # ベースコストさらに低減
        num_obstacles = np.random.randint(1, 3)  # 障害物数さらに減らす
        for _ in range(num_obstacles):
            cx = np.random.randint(0, grid_size)
            cy = np.random.randint(0, grid_size)
            radius = np.random.randint(grid_size//40, grid_size//25)  # さらに小さく
            y, x = np.ogrid[:grid_size, :grid_size]
            mask = (x - cx)**2 + (y - cy)**2 <= radius**2
            cost_map[mask] = np.random.uniform(0.4, 0.6)  # コストもさらに低減
        return cost_map
    ...
    def _generate_complex_terrain(self, seed: int, grid_size: int = 100) -> np.ndarray:
        ...
        """
        np.random.seed(seed)
        cost_map = np.ones((grid_size, grid_size), dtype=np.float32) * 0.35  # ベースコストさらに低減
        num_corridors = max(6, grid_size // 25)  # 通路をさらに増やす
        corridor_width = max(5, grid_size // 60)  # 通路をさらに広く
    ...
    def run_benchmark(self, scenarios: List[TestScenario], 
        ...
        """ベンチマーク実行"""
        if planners is None:
            planners = ["ADAPTIVE", "RRT_STAR", "SAFETY_FIRST", "HPA_STAR", "DSTAR_LITE"]  # 追加
        results = []
    ...
    def _run_adaptive_planner(self, scenario: TestScenario) -> BenchmarkResult:
        """
        ADAPTIVEプランナーで経路計画を実行（マップサイズに応じてタイムアウト延長）
        """
    ...
    def _run_rrt_star(self, scenario: TestScenario) -> BenchmarkResult:
        """
        RRT*で経路計画を実行（マップサイズに応じてタイムアウト延長）
        """
        if scenario.map_size == "SMALL":
            max_iterations = 5000
            timeout = 30.0
    ...
    def _run_hpa_star(self, scenario: TestScenario) -> BenchmarkResult:
        ...
        HPA*で経路計画を実行
        """
        # マップサイズに応じてクラスタサイズを調整
        if scenario.map_size == "SMALL":
            cluster_size = 25  # 100×100 → 4×4クラスタ
        elif scenario.map_size == "MEDIUM":
            cluster_size = 100  # 500×500 → 5×5クラスタ
        else:  # LARGE
            cluster_size = 200  # 1000×1000 → 5×5クラスタ
    ...
    def generate_statistics(self, results: List[BenchmarkResult]) -> Dict:
        ...
        """
        stats = {}
        # (プランナー, マップサイズ, 複雑度) でグループ化
        grouped = {}
        for result in results:
            # scenario_name例: scenario_small_simple_001 → map_size = SMALL
            try:
    ...
    def print_summary(self, stats: Dict):
        ...
        for key, data in sorted(stats.items()):
            # 末尾3要素のみ分割（シナリオ名等にアンダースコアが含まれても安全）
...
if __name__ == '__main__':
    print("="*60)
    print("Multi-Scale Path Planning Benchmark (5 Planners)")
    print("="*60)
    
    benchmark = PlannerBenchmark(output_dir="./benchmark_results")
    
    # マルチスケールシナリオ生成（各カテゴリ10シナリオに増加）
    print("\n[1/3] Generating multi-scale scenarios...")
    scenarios = benchmark.generate_test_scenarios({
        "SMALL": 30,    # 10 → 30（各複雑度10個ずつ）
        "MEDIUM": 30,   # 10 → 30
        "LARGE": 30     # 10 → 30
    })
    ...
    for map_size in ["SMALL", "MEDIUM", "LARGE"]:
        ...
        for complexity in ["SIMPLE", "MODERATE", "COMPLEX"]:
            count = sum(1 for s in scenarios 
                       if s.map_size == map_size and s.complexity == complexity)
    ...
    # 結果保存
    print("\n[3/3] Saving results...")
    benchmark.save_results(results, "multiscale_benchmark_results.json")
